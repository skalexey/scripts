...
function update_cmake_lists_job()
{
	...
	source log.sh
	local log_prefix="[update_cmake_lists_job]: "
	...
	file_replace "$dir/CMakeLists.txt" "set(DEPS \"\$ENV{build_deps}\")" "file(TO_CMAKE_PATH \"\$ENV{build_deps}\" DEPS)"
	...
	cd "$dir"
	git status
	git add CMakeLists.txt --patch
	git add */CMakeLists.txt --patch
	git status
	if need_to_commit; then
		if ask_user "Commit?"; then
			git commit -m "Auto update CMakeLists.txt"
		fi
	fi
