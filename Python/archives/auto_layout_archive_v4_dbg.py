...
class AutoLayout(WidgetBase(QHBoxLayout)):
	...
	class LayoutInfo:
		...
		def distribute_space(self):
			if not self.rows:
				...
				return
			log.debug(utils.method.msg_kw(f"========== Start ========== rect: {self.rect}, rows: {len(self.rows)}"))
			for row in self.rows:
				...
				for i, item_info in enumerate(row.items):
					item = item_info.item
					# internals = item_info.internals
					# size_hint = item.sizeHint()
					# log.debug(utils.method.msg_kw(f"Item {i} internals size_hint: {size_hint}"))
					# geometry_influencers = utils.pyside.collect_geometry_influencers(internals)
					# for influencer in geometry_influencers:
					# 	global_geometry = utils.pyside.global_geometry(influencer)
					# 	relative_geometry = utils.pyside.map_from_global(internals, global_geometry)
					# 	log.debug(utils.method.msg_kw(f"Item {i} influencer: {influencer}, global_geometry: {global_geometry}, relative_geometry: {relative_geometry}, bottom_right: {relative_geometry.bottomRight()}"))

					# not_hidden_children = utils.pyside.collect_not_hidden_children(internals)
					# for child in not_hidden_children:
					# 	relative_geometry = child.geometry()
					# 	log.debug(utils.method.msg_kw(f"Item {i} child: {child}, relative_geometry: {relative_geometry}, bottom_right: {relative_geometry.bottomRight()}"))
			...
			log.debug(utils.method.msg_kw("========== End =========="))
